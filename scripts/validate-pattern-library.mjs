...
for (const filePath of listMarkdown(patternsRoot)) {
  ...
  const previewPoster = String(data.preview_poster ?? '');
  ...
  if (previewPoster && !previewPoster.startsWith('media/')) {
    errors.push(`${file} preview_poster must live under media/: ${previewPoster}`);
  }
  ...
  if (/\.svg(?:$|\?)/i.test(previewPoster)) errors.push(`${file} uses SVG preview poster: ${previewPoster}`);
  ...
  if (previewPoster && !/\.(png|jpe?g)$/i.test(previewPoster)) {
    errors.push(`${file} preview_poster must be png, jpg, or jpeg: ${previewPoster}`);
  }
  ...
  if (previewPoster && !fs.existsSync(path.join(repoRoot, previewPoster))) {
    errors.push(`${file} preview_poster does not exist: ${previewPoster}`);
  }
